...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 3266 Vitest tests across 164 files and 63 checks in the chain as of 2026-08-14 (65 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
