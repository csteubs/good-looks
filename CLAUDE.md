...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 5577 Vitest tests across 298 files and 82 checks in the chain as of 2026-08-23 (84 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
