...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 6121 Vitest tests across 337 files and 94 checks in the chain as of 2026-08-26 (96 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
