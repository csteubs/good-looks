...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 5723 Vitest tests across 313 files and 85 checks in the chain as of 2026-08-24 (87 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
