...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 2569 Vitest tests across 130 files and 54 checks in the chain as of 2026-08-12 (56 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
