...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 5455 Vitest tests across 286 files and 80 checks in the chain as of 2026-08-22 (82 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
