...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 1959 Vitest tests across 100 files and 43 checks in the chain as of 2026-08-09 (45 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it; shell-drift needs both branches fetched, which only CI reliably has).
