...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 5599 Vitest tests across 302 files and 83 checks in the chain as of 2026-08-23 (85 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
