...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 1998 Vitest tests across 104 files and 47 checks in the chain as of 2026-08-09 (49 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
