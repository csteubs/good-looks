...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 3141 Vitest tests across 159 files and 61 checks in the chain as of 2026-08-13 (63 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
