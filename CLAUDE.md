...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 5803 Vitest tests across 317 files and 88 checks in the chain as of 2026-08-25 (90 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
