...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 3012 Vitest tests across 155 files and 60 checks in the chain as of 2026-08-12 (62 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
