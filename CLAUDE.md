...
main/services/agent/ the TRAINER AGENT (PR 3 of the bar plan): drives a live
                    ...
                    HTML, never a field's value) in page-summary.ts. ATTENDED —
                    every run starts from an explicit send, so no settings flag
                    and no egress check; the unattended suggestion strip (PR 4)
                    is where that treatment applies. e2e/agent-loop.spec.ts is
