...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 5334 Vitest tests across 276 files and 76 checks in the chain as of 2026-08-22 (78 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
