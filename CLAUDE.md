...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 3173 Vitest tests across 162 files and 62 checks in the chain as of 2026-08-13 (64 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
