...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 4084 Vitest tests across 195 files and 72 checks in the chain as of 2026-08-18 (74 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
