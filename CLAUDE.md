...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 5419 Vitest tests across 280 files and 79 checks in the chain as of 2026-08-22 (81 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
