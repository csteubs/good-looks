...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 2322 Vitest tests across 118 files and 52 checks in the chain as of 2026-08-10 (54 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
