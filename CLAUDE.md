...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 6048 Vitest tests across 333 files and 92 checks in the chain as of 2026-08-26 (94 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
