...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 3359 Vitest tests across 167 files and 65 checks in the chain as of 2026-08-14 (67 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
