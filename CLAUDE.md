...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 2120 Vitest tests across 113 files and 50 checks in the chain as of 2026-08-10 (52 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
