...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 1982 Vitest tests across 102 files and 45 checks in the chain as of 2026-08-09 (47 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it; shell-drift needs both branches fetched, which only CI reliably has).
