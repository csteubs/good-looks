...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 2924 Vitest tests across 150 files and 59 checks in the chain as of 2026-08-12 (61 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
