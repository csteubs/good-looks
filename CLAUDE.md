...
- **`check:*` scripts** predate Vitest and are kept, not migrated — they catch real bugs and a rewrite would risk that for tooling neatness. Plain assertions + a non-zero exit; no runner. Four are deliberately *source-level* (`check:ai-debug-scroll`, `check:scroll-layout`, `check:narrow-layout`, `check:clickable-chrome`) because they guard layout contracts that jsdom cannot observe — the last of those covers occlusion, which jsdom cannot see at all: an element covering a button leaves every rendered test passing.
