...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 5758 Vitest tests across 315 files and 86 checks in the chain as of 2026-08-25 (88 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
