...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 5517 Vitest tests across 289 files and 81 checks in the chain as of 2026-08-23 (83 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
