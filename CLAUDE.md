...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 6636 Vitest tests across 362 files and 97 checks in the chain as of 2026-09-03 (99 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
