...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 2879 Vitest tests across 147 files and 58 checks in the chain as of 2026-08-12 (60 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
