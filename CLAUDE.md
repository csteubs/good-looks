...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 5769 Vitest tests across 316 files and 87 checks in the chain as of 2026-08-25 (89 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
