...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 3380 Vitest tests across 169 files and 66 checks in the chain as of 2026-08-14 (68 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
