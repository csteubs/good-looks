...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 3775 Vitest tests across 184 files and 70 checks in the chain as of 2026-08-17 (72 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
