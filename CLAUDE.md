...
**Two systems, one command.** `npm run test:all` = the standalone `check:*` scripts, then Vitest. Both must pass. 2769 Vitest tests across 141 files and 57 checks in the chain as of 2026-08-12 (59 defined — `check:repo-hygiene` and `check:shell-drift` are deliberately outside it).
