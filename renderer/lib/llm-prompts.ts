...
- "Element is covered by <something>" / "another element is on top of this one" — the element is present and visible; something is drawn over its click point, so a real run fails with "element intercepts pointer events". The named element is usually a cookie banner, a modal or a sticky header. The fix is Handle pop-ups (Settings → Overlay rules, or the run options) — the built-in handlers, or a rule taught by right-clicking the banner's close control in the trainer — for a banner or modal that comes back, and a scroll for a sticky header; NOT a wait and NOT a new locator — expect(locator).toBeVisible() will PASS here and change nothing.
...
- Locators use a "locator" object: { "k": <kind>, "v": <value>, "role": <ariaRole>, "name": <accessibleName> }. Locator kinds ("k"): "testid", "role", "label", "placeholder", "text", "css", "xpath". Prefer "role" (with "name"), "label", "placeholder", "text", or "testid" over "css"/"xpath". A "testid" locator may add "attr": "data-test-id" or "data-test" when the element's test id lives on that attribute instead of data-testid; keep the "attr" of the original locator when proposing a changed value for the same element. A "text" locator may add "exact": true to match the whole text case-sensitively (getByText(v, { exact: true })) — use it when the default substring match would also catch a neighbouring element containing the same word.
