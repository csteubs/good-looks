...
| Host deep-link path | `app:getProjectPath` deleted (2026-09-03), and with it the last `app:*` channel | `app:getInfo`'s sibling, registered directly beneath it and left standing by that deletion's stated scope. It returned `__dirname/../..` as "the .glaze project path (used for deep links back to the host)" — but **there is no host**, which is the whole premise of this document, and the app's own `goodlooks://` deep links point INWARD: `shared/deep-link.mjs` parses them into a view from record ids and never into a path. Nothing called it, provably rather than by sampling — the preload's IPC funnel is a generic `glaze.ipc.invoke(channel, …)`, so every call site spells its channel as a literal, and the literal existed in exactly one place in the tree. Deleted rather than repointed: the app root is a main-process fact, and the one place that needs it (`main/services/mcp-install.ts`) resolves the same two levels beside the consumer that gives the value a contract. `registerHandlers()` now opens no `app:*` channel at all, which is what `handlers.test.ts` asserts — the property that would have caught this scaffold when the first one went. See DECISIONS 2026-09-03. |
