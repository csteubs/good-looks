...
| R6 | Record commit, branch and job provenance on each run | CI | **done 2026-08-26** | M |
...
| R24a | Attempt-key the run evidence (prerequisite for R24; unblocks R27/R29) | Stab | **done 2026-08-26** | M |
| R24 | Add opt-in retries, marked on the RunRecord | Stab | **done 2026-08-26** — CLI + Action; Routines still has none | L |
...
| R6 | missing | **done 2026-08-26** | `provenance?: RunProvenance` on `RunRecord`, one gate in `shared/run-provenance.mjs`, resolved once per runner and narrowed again by `run-history-store.append`. Explicit `GOOD_LOOKS_*` beats derived GitHub Actions, per field; every field untrusted, rejected rather than truncated, independently. **Carries the trigger fix it uncovered**: `mcp/run-tests.mjs` hard-coded `trigger: "mcp"`, so every `good-looks run` — this repo's own action included — was recorded as MCP-driven since R3. `cli` joins the vocabulary; `createRunner` takes the value with no default. `check:mcp-parity` §14/§18 + a real run in `check:cli-exit` |
| R12 | missing | **done 2026-08-26** | `good-looks ingest DIR`. Gate in `shared/run-ingest.mjs`, disk work in `cli/ingest.mjs`, written through `saveRunRecords` so the cap and pruned tally stay one implementation. Deduped by id, so re-running ingests nothing. `logFile` is never carried — the store's `readLog`/`searchLogs` read it unbounded, so a foreign path there is readable through the app's search box; the gate returns none and the caller derives one from a validated id. `ingestedAt` marks a run this machine did not perform, which is what lets the cost and duration readouts exclude another machine's hardware. `check:run-ingest` + 38 tests |
| R24a | missing | **done 2026-08-26** | `shared/attempt-artifacts.mjs` — attempt 0 keeps the run directory, later attempts get `attempt-<n>/`. That asymmetry is the decision: attempt 0 is the FAILING attempt (a retry only follows a failure) and is where every existing reader already points. The capture fixture reads `testInfo.retry` and roots its context there; both reporter copies emit `result.retry`, which was present and underscore-prefixed all along; `stepStatusMaps` is keyed by attempt and the replay is built from attempt 0's. The trace gate moved off the exit code — `retain-on-failure` keeps the failing attempt's trace and a retried pass exits 0, so the retry that made the run green deleted the evidence of why it had not been. Playwright's `-retry<n>` directory naming was read out of the installed `workerProcessEntry.js`, not assumed; this repo's own comment had it as a nested `retryN/`. NOT done: `attempt` on the `runner:step` payload (backfillable, and nothing renders it until R24) and an `artifactStore.listAttempts` (a read API with no reader). `check:retry-evidence` + `e2e/retry-evidence.spec.ts` + 32 tests |
| R24 | missing | **done 2026-08-26** | `--retries N` on the CLI and the Action; `attempt` and `passedOnRetry` on `RunRecord`, derived from the marker stream R24a made attempt-aware. The counting rule is `shared/run-attempts.mjs`: a retried pass is PASSED in the outcome tally and FAILED in the transition series. The verdict check sits ABOVE `failed === 0`, which is where this could most easily have shipped wrong — `verdictFor` reads outcomes, so a test needing a retry every run reached "stable", the exact behaviour ROUTINES refused the feature over, with every new field present and correct. Surfaced three existing inconsistencies: `reviewReason` divided `flakeRuns` by a `failures` denominator that no longer contains it, `never-caught` would have described a test that recovers on every run, and the cost panel said "none yet" above a non-zero flake spend. `good-looks ingest` carries both fields — `check:run-ingest` could not have caught their absence, since both are optional and the gate only owes what `RunRecord` REQUIRES. NOT done: an app-side flag (a Settings pane and a per-test pin, its own change) and a Routines policy (`stopRoutine`/`skipGroup` key on "a step failed", which a retried pass escapes). `check:retry-evidence` §6-7 + `check:flake-analysis` rows + a real-marker row in `e2e/retry-evidence.spec.ts` |
