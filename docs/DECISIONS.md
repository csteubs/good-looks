...
same kind of thing as the "7 of 7 reporting" subtitle already in that header, so
...
- **The sentence truncates; the subtitle does not.** Both header items shrink by
  default, so the first narrow window clipped BOTH — "7 of 7 rep…" beside a
...
produce ("1 category needs attention, 1 is clean, and 1 has never been
measured."): the header stays 32px and the title stays whole at every width.
