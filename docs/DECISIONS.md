...
- **Rejected: a `check:*` guard that every `sendToMain` channel has a renderer subscriber.** It would have caught this, and the sweep found two more unsubscribed channels — `recorder:loadFailed` and `recorder:healSuggestion`. Both turn out to be *redundant* rather than broken: the renderer reads `state.loadFailed` off `recorder:state`, and heals ride along on the replay-step events. So the check cannot be turned on without first deciding whether those pushes should be deleted or consumed, which is a separate change. Left as a follow-up rather than shipped half-enforced or with an allowlist that hides the very thing it looks for.
