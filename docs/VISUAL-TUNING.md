...
> **Carried onto `main` 2026-08-14, and still open.** Written on
> `claude/transcribe-notes-plan-work-e36d77`, which never merged — so its
> headline recommendation went with it. Re-verified against `main` on
> 2026-08-14: **animations are still not disabled at capture** (no `animations`
> option in `visual-pipeline.ts` or `page-actions.ts`), which is the change this
> document argues is the actual fix. "Accept all" survived as a button, now
> behind a confirm that says it repins every baseline. Read this as live work,
> not history.
